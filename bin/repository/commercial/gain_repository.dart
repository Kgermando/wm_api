...
class GainRepository {
  ...
  Future<List<CourbeGainModel>> getAllDataChartDay() async {
    ...
    var querySQL =
        "SELECT EXTRACT(HOUR FROM \"created\" ::TIMESTAMP), SUM(\"sum\"::FLOAT) FROM $tableName DATE(\"created\") >= CURRENT_DATE AND DATE(\"created\") < CURRENT_DATE + INTERVAL '1 DAY'  GROUP BY EXTRACT(HOUR FROM \"created\" ::TIMESTAMP) ORDER BY EXTRACT(HOUR FROM \"created\" ::TIMESTAMP) ASC ;";
